...
<p align="center">
  <img src="https://img.shields.io/badge/version-13.22.1-blue?logo=semver&logoColor=white" alt="version">
...
- **AI 代码示例（v12.0.0 / v13.22.1）**：前端按鉴权×语言分片请求（最多 27 片）；详细文档与单片代码均支持 SSE 流式回填；系统设置可开单线程/并行；进程实时显示当前任务
...
- AI 代码示例单片改为 SSE 流式回填（后台/用户中心）
- 外链图标 http→https、失败字母兜底；CSP `upgrade-insecure-requests`（E212）
- 修复迁移 5.0.0 全表抹 kind；友链页/页脚硬上限；移除临时「改回伙伴」（E211）
