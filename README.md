...
- **积分计费与充值（v3.33+ / v3.34）**：接口收费扣积分；用户充值中心扫码支付；订单管理与积分变动分栏；码支付回调 `codepay.php`
...
├── codepay.php                 # 码支付公网入口（PATH_INFO：/notify|/return → core/play/codeplay）
...
- 用户充值中心 / 积分变动 UI 重做；码支付回调迁入 `core/play/codeplay`，入口 `codepay.php`
