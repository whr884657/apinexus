...
(function () {
    ...
    function showAlert(type, msg) {
        ...
        var base = alertEl.getAttribute('data-alert-base') || '';
        if (!base) {
            base = alertEl.className || '';
            alertEl.setAttribute('data-alert-base', base);
        }
        var tone = (type === 'error') ? 'alert-error is-error' : 'alert-success is-success';
        alertEl.className = (base + ' alert ' + tone).replace(/\s+/g, ' ').trim();
    ...
    function setFetchStatus(msg, tone) {
        ...
        statusEl.className = 'applylink-fetch-status st-apply-fetch-status'
            + (tone === 'ok' ? ' is-ok' : '')
            + (tone === 'err' ? ' is-err' : '');
    }
