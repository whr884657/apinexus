...
echo '<script>document.body.classList.add("theme-slate-auth");</script>';
?>
    ...
    <div class="right">
        <div class="form-box">
            ...
            <form id="loginForm" method="post" action="" novalidate>
                ...
                <div class="row">
                    <label class="remember"><input type="checkbox" id="rememberCredentials" value="1"> 记住账号密码</label>
                ...
                <div class="oauth-section">
                    ...
                    <div class="oauth-section__icons">
                        <?php if (!empty($oauthProviders['qq'])): ?>
                            <a href="<?php echo vs_e($base); ?>/user/oauth/start.php?provider=qq" class="oauth-icon" title="QQ 登录"><img src="<?php echo vs_e($base); ?>/assets/img/QQ.svg" alt="" width="22" height="22"></a>
                        <?php endif; ?>
                        <?php if (!empty($oauthProviders['gitee'])): ?>
                            <a href="<?php echo vs_e($base); ?>/user/oauth/start.php?provider=gitee" class="oauth-icon" title="Gitee 登录"><img src="<?php echo vs_e($base); ?>/assets/img/gitee.svg" alt="" width="22" height="22"></a>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
                <div class="divider">还没有账号？<a href="<?php echo vs_e($base); ?>/user/register">立即注册</a></div>
            </form>
        </div>
    </div>
</div>
...
(function () {
    ...
    function showMessage(text, type) {
        ...
        if (!messageEl) return;
        messageEl.textContent = text; messageEl.className = 'form-message form-message--' + type; messageEl.hidden = false;
    }
    ...
    form.addEventListener('submit', function (e) {
        ...
        var body = new FormData(form); body.append('action', 'login');
        fetch(form.action || window.location.href, { method: 'POST', body: body, credentials: 'same-origin' })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (data.code === 1) { showMessage(data.msg || '登录成功', 'success'); if (data.url) setTimeout(function () { window.location.href = data.url; }, 800); }
