...
if ($registerOpen) {
    vs_slate_auth_shell_start('注册账号', $subTitle);
} else {
    vs_slate_auth_shell_start('已停止注册', $registerClosedSub);
}
?>
...
<?php if (!$registerOpen): ?>
    <div class="st-auth__msg st-auth__msg--error" role="status"><?php echo vs_e($registerClosedDetail); ?></div>
...
<form id="registerForm" method="post" action="" novalidate>
    ...
    <div class="st-auth__field st-auth__field--role-segment">
        <div class="st-role-segment" id="roleSegment">
            <div class="st-role-segment__track" id="roleSegmentTrack" data-role="user" role="radiogroup" aria-label="账号类型">
                ...
                <button type="button" class="st-role-segment__btn is-active" data-role="user" aria-pressed="true" <?php echo $dis; ?>>普通用户</button>
                <button type="button" class="st-role-segment__btn" data-role="developer" aria-pressed="false" <?php echo $dis; ?>>开发者</button>
            </div>
    ...
    <div class="st-auth__field">
        <input class="st-auth__input" id="email" name="email" type="email" placeholder="请输入邮箱" autocomplete="email" maxlength="64" required aria-label="邮箱" <?php echo $dis; ?>>
    </div>
    <?php vs_captcha_field(Captcha::SCENE_USER_REGISTER, 'local'); ?>
    <?php if ($emailVerify): ?>
    <div class="st-auth__field">
        <div class="st-auth__group">
            <input class="st-auth__input" id="code" name="code" type="text" placeholder="请输入邮箱验证码" autocomplete="one-time-code" maxlength="6" inputmode="numeric" pattern="[0-9]*" aria-label="邮箱验证码" <?php echo $dis; ?>>
            <button type="button" class="st-auth__code-btn" id="sendCodeBtn" <?php echo $dis; ?>>获取验证码</button>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="st-auth__field">
        <div class="st-auth__pw-wrap">
            <input class="st-auth__input" id="password" name="password" type="password" placeholder="请设置密码（至少6位）" autocomplete="new-password" maxlength="64" required aria-label="密码" <?php echo $dis; ?>>
            <?php echo vs_slate_pw_toggle_html(); ?>
        </div>
    </div>
    <div class="st-auth__field">
        <input class="st-auth__input" id="confirm_password" name="confirm_password" type="password" placeholder="请再次输入密码" autocomplete="new-password" maxlength="64" required aria-label="确认密码" <?php echo $dis; ?>>
    </div>
    <button type="submit" class="st-auth__submit" id="submitBtn" <?php echo $dis; ?>>立即注册</button>
