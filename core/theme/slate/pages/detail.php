...
if (!$notFound) {
    if ($paramsRaw !== '') {
        $stTabs[] = array('id' => 'params', 'label' => '请求参数');
    }
    if (!empty($api['response'])) {
        $stTabs[] = array('id' => 'response', 'label' => '返回示例');
    }
    $stTabs[] = array('id' => 'doc', 'label' => '详细文档');
...
?>
<?php require_once dirname(__DIR__) . '/lib/bootstrap.php'; ?>
<main class="st-main st-detail-page" id="apiDetailPage"
...
<section class="st-detail">
    ...
    <?php
    $detailImgBase = rtrim(ThemeManager::assetUrl('slate', 'assets/img/'), '/') . '/';
    ...
    <?php if ($isDisabled): ?>
    <div class="st-detail__notice st-detail__notice--danger">该接口已被禁用，调用地址与可调用信息已隐藏，暂时无法请求。</div>
    <?php elseif ($isMaintenance): ?>
    <div class="st-detail__notice st-detail__notice--warn">当前接口维护中，暂时无法调用。</div>
    <?php endif; ?>

    <div class="st-detail__panel st-detail__info">
        <h2 class="st-detail__h">接口信息</h2>
        ...
        <div class="st-detail-info-grid info-grid">
            <div class="info-item">
                <div class="info-label">Method</div>
                <div class="info-value info-value--method"><?php echo vs_e(isset($api['method_label']) ? $api['method_label'] : strtoupper($primaryMethod)); ?></div>
            </div>
            <div class="info-item">
    ...
    <!-- 下半：水平 Tab（桌面 / 移动同为可横向滚动按钮行） -->
    <div class="st-detail-tabs" role="tablist" aria-label="接口详情分区">
        <?php foreach ($stTabs as $ti => $tab): ?>
        <button type="button"
                class="st-detail-tab<?php echo $tab['id'] === $stFirstTab ? ' is-active' : ''; ?>"
                role="tab"
                aria-selected="<?php echo $tab['id'] === $stFirstTab ? 'true' : 'false'; ?>"
                data-st-tab="<?php echo vs_e($tab['id']); ?>"><?php echo vs_e($tab['label']); ?></button>
        <?php endforeach; ?>
    </div>

    <div class="st-detail-panels">

        <?php if ($paramsRaw !== ''): ?>
        <div class="st-detail-panel st-detail__panel" data-st-panel="params" role="tabpanel"<?php echo $stFirstTab === 'params' ? '' : ' hidden'; ?>>
        ...
        <div class="st-detail-panel st-detail__panel" data-st-panel="response" role="tabpanel"<?php echo $stFirstTab === 'response' ? '' : ' hidden'; ?>>
            <div class="detail-section-title detail-section-title--tools">
        ...
        <div class="st-detail-panel st-detail__panel" data-st-panel="doc" role="tabpanel"<?php echo $stFirstTab === 'doc' ? '' : ' hidden'; ?>>
            <h2 class="st-detail__h">详细文档</h2>
            <?php if (!empty($api['doc'])): ?>
            <div class="markdown-body vs-md-body st-md detail-md is-parsed st-detail__doc"><?php echo slate_md_render((string) $api['doc']); ?></div>
            <?php else: ?>
            <p class="detail-empty-hint">暂无详细文档</p>
            <?php endif; ?>
        </div>

        <div class="st-detail-panel st-detail__panel" data-st-panel="quickstart" role="tabpanel" id="detailQuickstartCard"<?php echo $stFirstTab === 'quickstart' ? '' : ' hidden'; ?>>
            <h2 class="st-detail__h">快速上手</h2>
        ...
        <div class="st-detail-panel st-detail__panel" data-st-panel="playground" role="tabpanel" id="detailPlayground"<?php echo $stFirstTab === 'playground' ? '' : ' hidden'; ?>>
            <h2 class="st-detail__h">在线测试</h2>
            <?php if ($isDisabled): ?>
            <div class="st-detail__notice st-detail__notice--danger">接口已禁用，暂不可测试。</div>
            <?php elseif ($isMaintenance): ?>
            <div class="st-detail__notice st-detail__notice--warn">维护中，暂不可测试。</div>
            ...
            <div class="playground-grid">
                <div class="playground-pane">
                    ...
                    <button type="button" class="st-detail__btn st-detail__btn--primary playground-send" id="pgSendBtn">发送请求</button>
                </div>
                <div class="playground-pane">
        ...
        <div class="st-detail-panel st-detail__panel" data-st-panel="feedback" role="tabpanel" id="detailFeedbackCard"
             ...
             <?php echo $stFirstTab === 'feedback' ? '' : 'hidden'; ?>>
            <h2 class="st-detail__h">接口反馈</h2>
            ...
            <form id="detailFeedbackForm" class="detail-feedback" method="post" action="" novalidate>
                ...
                <div class="detail-feedback__foot">
                    <button type="submit" class="st-detail__btn st-detail__btn--primary detail-feedback__submit" id="detailFeedbackBtn">提交反馈</button>
                </div>
            </form>
            <?php endif; ?>
        </div>
...
<script src="<?php echo vs_e(ThemeManager::assetUrl('slate', 'assets/js/playground-response.js')); ?>?v=<?php echo vs_e(VS_VERSION); ?>" defer></script>
<script src="<?php echo vs_e(ThemeManager::assetUrl('slate', 'assets/js/pages/detail-quickstart.js')); ?>?v=<?php echo vs_e(VS_VERSION); ?>" defer></script>
<script src="<?php echo vs_e(ThemeManager::assetUrl('slate', 'assets/js/pages/detail.js')); ?>?v=<?php echo vs_e(VS_VERSION); ?>" defer></script>
