...
if ($registerOpen) {
    TH5_auth_shell_start('创建账号', $subTitle);
} else {
    TH5_auth_shell_start('已停止注册', $registerClosedSub);
}
?>
...
<?php if (!$registerOpen): ?>
    <div class="th5-auth__msg th5-auth__msg--error" role="status"><?php echo vs_e($registerClosedDetail); ?></div>
    <div class="th5-auth__foot"><a href="<?php echo vs_e($base); ?>/user/login">返回登录</a></div>
<?php else: ?>

<?php if ($emailVerify && !$mailEnabled): ?>
    <div class="th5-auth__msg th5-auth__msg--error"><?php echo vs_e($mailDisabledMsg); ?></div>
<?php endif; ?>
...
<form id="registerForm" method="post" action="" novalidate>
    ...
    <div class="th5-auth__field th5-auth__role">
        <div class="th5-auth__role-track" id="roleSegmentTrack" data-role="user" role="radiogroup" aria-label="账号类型">
            <span class="th5-auth__role-thumb" aria-hidden="true"></span>
            <button type="button" class="th5-auth__role-btn is-active" data-role="user" aria-pressed="true" <?php echo $dis; ?>>普通用户</button>
            <button type="button" class="th5-auth__role-btn" data-role="developer" aria-pressed="false" <?php echo $dis; ?>>开发者</button>
        </div>
    ...
    <div class="th5-auth__field">
        <input class="th5-auth__input" id="username" name="username" type="text" placeholder="用户名（3～50 个字符）" autocomplete="username" maxlength="50" required aria-label="用户名" <?php echo $dis; ?>>
    </div>
    <div class="th5-auth__field">
        <input class="th5-auth__input" id="email" name="email" type="email" placeholder="邮箱" autocomplete="email" maxlength="64" required aria-label="邮箱" <?php echo $dis; ?>>
    </div>
    <?php vs_captcha_field(Captcha::SCENE_USER_REGISTER, 'local'); ?>
    <?php if ($emailVerify): ?>
    <div class="th5-auth__field">
        <div class="th5-auth__group">
            <input class="th5-auth__input" id="code" name="code" type="text" placeholder="邮箱验证码" autocomplete="one-time-code" maxlength="6" inputmode="numeric" pattern="[0-9]*" aria-label="邮箱验证码" <?php echo $dis; ?>>
            <button type="button" class="th5-auth__code-btn" id="sendCodeBtn" <?php echo $dis; ?>>获取验证码</button>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="th5-auth__field">
        <div class="th5-auth__pw-wrap">
            <input class="th5-auth__input" id="password" name="password" type="password" placeholder="密码（至少6位）" autocomplete="new-password" maxlength="64" required aria-label="密码" <?php echo $dis; ?>>
            <?php echo TH5_pw_toggle_html(); ?>
        </div>
    </div>
    <div class="th5-auth__field">
        <input class="th5-auth__input" id="confirm_password" name="confirm_password" type="password" placeholder="再次输入密码" autocomplete="new-password" maxlength="64" required aria-label="确认密码" <?php echo $dis; ?>>
    </div>
    <button type="submit" class="th5-auth__submit" id="submitBtn" <?php echo $dis; ?>>立即注册</button>
    <div class="th5-auth__foot">已有账号？<a href="<?php echo vs_e($base); ?>/user/login">返回登录</a></div>
</form>
...
(function () {
    ...
    function setRole(role, showHint) {
        ...
        roleTrack.querySelectorAll('.th5-auth__role-btn').forEach(function (btn) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var runRegister = function () {
            if (window.TH5AuthSetLoading) window.TH5AuthSetLoading(submitBtn, true);
            ...
            Promise.resolve(post)
                ...
                .finally(function () { if (window.TH5AuthSetLoading) window.TH5AuthSetLoading(submitBtn, false); });
        };
