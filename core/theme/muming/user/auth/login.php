...
TH5_auth_shell_start('欢迎回来', '登录后继续使用平台能力');
?>
...
<form id="loginForm" method="post" action="" novalidate data-login-mode="password">
    ...
    <div id="modePassword" data-mode-panel="password">
        <div class="th5-auth__field">
            <input class="th5-auth__input" id="username" name="username" type="text" placeholder="用户名或邮箱" autocomplete="username" maxlength="64" required aria-label="用户名或邮箱">
        </div>
        <div class="th5-auth__field">
    ...
    <div id="modeCode" data-mode-panel="code" hidden>
        <div class="th5-auth__field">
            <input class="th5-auth__input" id="email" name="email" type="email" placeholder="注册邮箱" autocomplete="email" maxlength="64" aria-label="注册邮箱" <?php echo $mailEnabled ? '' : 'disabled'; ?>>
        </div>
    </div>
    ...
    <div id="modeCodeCode" data-mode-panel="code" hidden>
        <div class="th5-auth__field">
            <div class="th5-auth__group">
                <input class="th5-auth__input" id="code" name="code" type="text" placeholder="邮箱验证码" autocomplete="one-time-code" maxlength="6" inputmode="numeric" pattern="[0-9]*" aria-label="邮箱验证码" <?php echo $mailEnabled ? '' : 'disabled'; ?>>
                <button type="button" class="th5-auth__code-btn" id="sendCodeBtn" <?php echo $mailEnabled ? '' : 'disabled'; ?>>获取验证码</button>
            </div>
        </div>
    </div>

    <?php /* 观感抄本=主题1/2：切换行在人机验证下方；防 E277 用 button + z-index */ ?>
    <div class="th5-auth__row">
        <button type="button" class="th5-auth__mode-toggle" id="toggleLoginMode">验证码登录</button>
        <a href="<?php echo vs_e($base); ?>/user/forgot">忘记密码？</a>
    </div>

    <button type="submit" class="th5-auth__submit" id="loginBtn">登 录</button>

    <?php if (!empty($oauthButtons)): ?>
    <div class="th5-auth__oauth">
        <div class="th5-auth__oauth-label">第三方登录</div>
        <div class="th5-auth__oauth-icons">
            <?php foreach ($oauthButtons as $btn): ?>
                ...
                ?>
                <a href="<?php echo vs_e($btnUrl); ?>" title="<?php echo vs_e($btnLabel); ?> 登录" aria-label="<?php echo vs_e($btnLabel); ?> 登录">
                    <img src="<?php echo vs_e($btnIcon); ?>" alt="<?php echo vs_e($btnLabel); ?>" width="22" height="22">
                </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <?php if ($registerOpen): ?>
    <div class="th5-auth__foot">还没有账号？<a href="<?php echo vs_e($base); ?>/user/register">立即注册</a></div>
    <?php endif; ?>
</form>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (mode === 'code') {
            ...
            if (window.TH5AuthSetLoading) window.TH5AuthSetLoading(loginBtn, true);
            ...
            }).finally(function () { if (window.TH5AuthSetLoading) window.TH5AuthSetLoading(loginBtn, false); });
            return;
        }
        ...
        if (window.VsCaptcha && window.VsCaptcha.enabled && window.VsCaptcha.ensure) {
            window.VsCaptcha.ensure(form).then(runLogin).catch(function (err) {
                showMessage((err && err.message) || '请先完成行为验证', 'error');
                if (window.TH5AuthSetLoading) window.TH5AuthSetLoading(loginBtn, false);
            });
        } else { runLogin(); }
    });
})();
