/**
 * 主题 five · API 详情页（tabs / 复制 MD / 参数模式 / playground / 反馈）
 */
(function () {
    'use strict';
    ...
    /* —— Tabs —— */
    (function initTabs() {
        var tabs = page.querySelectorAll('[data-th5-tab]');
        var panels = page.querySelectorAll('[data-th5-panel]');
        ...
        function show(id) {
            ...
            Array.prototype.forEach.call(tabs, function (t) {
                if (t.getAttribute('data-th5-tab') === id) ok = true;
            });
            if (!ok) return;
            Array.prototype.forEach.call(tabs, function (t) {
                var on = t.getAttribute('data-th5-tab') === id;
                t.classList.toggle('is-active', on);
                t.setAttribute('aria-selected', on ? 'true' : 'false');
            });
            Array.prototype.forEach.call(panels, function (p) {
                p.hidden = p.getAttribute('data-th5-panel') !== id;
            });
            // 禁止改写地址栏 hash（E358/E368）：切换 Tab 不得往 URL 追加 #params 等碎片
        }

        Array.prototype.forEach.call(tabs, function (t) {
            t.addEventListener('click', function () {
                show(t.getAttribute('data-th5-tab'));
            });
        });

        // 仅首次进入时认遗留 hash 深链并立刻清掉碎片
    ...
    function getMethod() {
        if (methodSelector) {
            var active = methodSelector.querySelector('.th5-pill.is-active,[data-method].is-active');
