...
$metaUrl = $vsBase . '/core/theme/muming/api/sitemeta.php';
?>
<section class="th5-page">
  <div class="th5-page__inner max-w-2xl mx-auto px-4 sm:px-5 lg:px-8">
    <div class="text-xs font-mono uppercase tracking-widest text-muted mb-3">/ 申请友链</div>
    <h1 class="font-display font-bold tracking-tight th5-page__title">申请友情链接</h1>
    <p class="th5-page__lead text-fg-2">请先在贵站添加本站信息，再填写下方表单。</p>

    <?php if (!empty($siteCard)): ?>
    <div class="th5-panel card mb-6">
    ...
    <div id="applyAlert" class="th5-alert" role="alert" hidden></div>

    <form id="applyLinkForm" class="th5-panel card th5-form" method="post" action="<?php echo vs_e($vsBase); ?>/applylink" novalidate>
      <input type="hidden" name="csrf_token" value="<?php echo vs_e($csrf); ?>">
      <input type="hidden" name="action" value="apply">

      <label class="th5-field">
        <span>网站地址</span>
        <div class="th5-field-row">
        ...
        <span class="th5-field-hint" id="applyFetchStatus"></span>
      </label>
      ...
      <label class="th5-field">
        <span>简介（可选）</span>
        <textarea class="input th5-textarea" id="applyDesc" name="description" rows="3" maxlength="200" placeholder="一句话介绍"></textarea>
      </label>
