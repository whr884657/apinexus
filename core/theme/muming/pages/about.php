...
if ($hasAbout) {
    ...
    if ($raw !== '') {
        $aboutBodyHtml = TH5_md_render($raw);
...
<section class="th5-page">
  <div class="th5-page__inner max-w-3xl mx-auto px-4 sm:px-5 lg:px-8">
    ...
    <h1 class="font-display font-bold tracking-tight th5-page__title"><?php echo vs_e($aboutTitle); ?></h1>
    <?php if ($hasAbout && $aboutSummary !== ''): ?>
      <p class="th5-page__lead text-fg-2"><?php echo vs_e($aboutSummary); ?></p>
    <?php elseif (!$hasAbout): ?>
      <p class="th5-page__lead text-fg-2">了解 <?php echo vs_e($siteName); ?></p>
    <?php endif; ?>

    <div class="th5-panel card mt-8">
      <?php if ($hasAbout): ?>
        <div class="markdown-body vs-md-body th5-md"><?php echo $aboutBodyHtml; ?></div>
      <?php else: ?>
