...
<section class="th3-page" id="profilePage" data-ping-url="<?php echo vs_e($pingUrl); ?>">
  <div class="th3-page__inner max-w-5xl mx-auto px-4 sm:px-5 lg:px-8">
    <?php if ($notFound): ?>
      ...
      <p class="th3-page__lead text-fg-2">该贡献者主页不存在或已隐藏。</p>
      <a class="btn-primary mt-6 inline-flex" href="<?php echo vs_e($vsBase); ?>/contributors">返回贡献者</a>
    <?php else: ?>
      <div class="text-xs font-mono uppercase tracking-widest text-muted mb-3">/ 主页</div>
      <div class="th3-profile-head card">
        <?php if (!empty($profile['avatar'])): ?>
          <img class="th3-profile-head__avatar" src="<?php echo vs_e($profile['avatar']); ?>" alt="" width="72" height="72">
        <?php else: ?>
          <span class="th3-profile-head__avatar th3-profile-head__avatar--text"><?php echo vs_e(isset($profile['letter']) ? $profile['letter'] : '?'); ?></span>
        <?php endif; ?>
        <div>
          ...
          ?></p>
          <div class="th3-contrib-card__stats mt-3">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-10 mb-4">
        ...
        <input id="apiSearch" class="input th3-profile-search" type="search" placeholder="搜索接口…" autocomplete="off">
      </div>
        ...
        <?php else: ?>
          <?php foreach ($apis as $api): ?>
            ...
            <a class="th3-profile-api card"
              ...
              <div class="th3-profile-api__top">
                <span class="tag free"><?php echo vs_e($m0); ?></span>
                <span class="tag <?php echo $points > 0 ? 'points' : 'free'; ?>"><?php echo vs_e($billing); ?></span>
              </div>
              <div class="th3-profile-api__name"><?php echo vs_e(isset($api['name']) ? $api['name'] : $initial); ?></div>
              <div class="th3-profile-api__meta text-xs text-muted">
...
<script src="<?php echo vs_e(ThemeManager::assetUrl('three', 'assets/js/pages/profile.js')); ?>?v=<?php echo vs_e(VS_VERSION); ?>" defer></script>
