<?php
/**
 * 主题 three · API 详情（自研 th3 视觉，能力对齐默认主题）
 */
...
if (!$notFound) {
    if ($paramsRaw !== '') {
        $th3Tabs[] = array('id' => 'params', 'label' => '请求参数');
    }
    if (!empty($api['response'])) {
        $th3Tabs[] = array('id' => 'response', 'label' => '返回示例');
    }
    $th3Tabs[] = array('id' => 'doc', 'label' => '详细文档');
    $th3Tabs[] = array('id' => 'quickstart', 'label' => '快速上手');
    $th3Tabs[] = array('id' => 'playground', 'label' => '在线测试');
    $th3Tabs[] = array('id' => 'feedback', 'label' => '接口反馈');
}
$th3FirstTab = isset($th3Tabs[0]['id']) ? (string) $th3Tabs[0]['id'] : '';
...
?>
<section class="th3-page th3-detail" id="apiDetailPage"
  ...
  <div class="th3-page__inner max-w-5xl mx-auto px-4 sm:px-5 lg:px-8">
    ...
    <?php if ($notFound): ?>
      <h1 class="font-display font-bold tracking-tight th3-page__title">接口不存在</h1>
      <p class="th3-page__lead text-fg-2">该接口不存在、未通过审核或已下架。</p>
      <a class="btn-primary inline-flex mt-6" href="<?php echo vs_e($vsBase); ?>/apis">返回全部接口</a>
    <?php else: ?>

      <div class="th3-detail__head">
        <div class="th3-detail__title-row">
          <h1 class="font-display font-bold tracking-tight th3-page__title m-0"><?php echo vs_e($api['name']); ?></h1>
          <div class="th3-detail__head-actions">
            <?php if ($isDisabled): ?>
        ...
        <?php if (!empty($api['desc'])): ?>
          <p class="th3-page__lead text-fg-2 mt-3"><?php echo vs_e($api['desc']); ?></p>
        <?php endif; ?>
      </div>

      <?php if ($isDisabled): ?>
        <div class="th3-alert th3-alert--danger mt-4">该接口已被禁用，调用地址已隐藏，暂时无法请求。</div>
      <?php elseif ($isMaintenance): ?>
        <div class="th3-alert th3-alert--warn mt-4">当前接口维护中，暂时无法调用。</div>
      <?php endif; ?>
      ...
      <div class="th3-panel card mt-8">
        ...
        <div class="th3-info-grid">
          <div class="th3-info-item">
            <div class="th3-info-label">方法</div>
            <div class="th3-info-value"><?php echo vs_e(isset($api['method_label']) ? $api['method_label'] : strtoupper($primaryMethod)); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">分类</div>
            <div class="th3-info-value"><?php echo vs_e(!empty($api['category_name']) ? $api['category_name'] : '未分类'); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">计费</div>
            <div class="th3-info-value"><?php echo vs_e($chargeDetailLabel); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">KEY</div>
            <div class="th3-info-value"><?php echo vs_e($keyLabel); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">调用</div>
            <div class="th3-info-value"><?php echo vs_e($callsLabel); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">QPM</div>
            <div class="th3-info-value"><?php echo vs_e(isset($api['qpm_label']) ? $api['qpm_label'] : '不限制'); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">鉴权</div>
            <div class="th3-info-value"><?php echo vs_e($authWayLabel); ?></div>
          </div>
          <div class="th3-info-item">
            <div class="th3-info-label">作者</div>
            <div class="th3-info-value">
      ...
      <div class="th3-tabs mt-8" role="tablist" aria-label="接口详情分区">
        <?php foreach ($th3Tabs as $tab): ?>
          <button type="button"
                  class="th3-pill<?php echo $tab['id'] === $th3FirstTab ? ' is-active' : ''; ?>"
                  role="tab"
                  aria-selected="<?php echo $tab['id'] === $th3FirstTab ? 'true' : 'false'; ?>"
                  data-th3-tab="<?php echo vs_e($tab['id']); ?>"><?php echo vs_e($tab['label']); ?></button>
        <?php endforeach; ?>
      </div>

      <div class="th3-panels mt-4">

        <?php if ($paramsRaw !== ''): ?>
        <div class="th3-panel card" data-th3-panel="params" role="tabpanel"<?php echo $th3FirstTab === 'params' ? '' : ' hidden'; ?>>
          <div class="th3-panel__tools">
            <h2 class="font-display font-semibold text-lg m-0">请求参数</h2>
            <div class="th3-panel__actions">
          ...
          <?php if ($hasParamsTable): ?>
            <div class="th3-table-wrap mt-4" id="paramsTableMode">
              <table class="th3-table">
                <thead>
            ...
            <div class="th3-code mt-4" id="paramsJsonMode" hidden>
        ...
        <div class="th3-panel card" data-th3-panel="doc" role="tabpanel"<?php echo $th3FirstTab === 'doc' ? '' : ' hidden'; ?>>
          <h2 class="font-display font-semibold text-lg m-0 mb-4">详细文档</h2>
          <?php if (!empty($api['doc'])): ?>
            <div class="markdown-body vs-md-body th3-md"><?php echo th3_md_render((string) $api['doc']); ?></div>
          <?php else: ?>
            <p class="text-fg-2 m-0">暂无详细文档</p>
          <?php endif; ?>
        </div>

        <div class="th3-panel card" data-th3-panel="quickstart" role="tabpanel" id="detailQuickstartCard"<?php echo $th3FirstTab === 'quickstart' ? '' : ' hidden'; ?>>
          ...
          <?php else: ?>
            ...
            <div class="th3-qs-auth" id="detailQsAuthTabs" role="tablist" aria-label="鉴权方式">
              <?php foreach ($qsBundle['auths'] as $ai => $authId): ?>
                ...
                ?>
                <button type="button"
                        class="th3-pill<?php echo $ai === 0 ? ' is-active' : ''; ?>"
                        role="tab"
            ...
            <?php endif; ?>
            <div class="th3-qs" id="detailQuickstart"
                 data-qs-count="<?php echo count($qsSamples); ?>"
                 data-qs-multi-auth="<?php echo $qsShowAuthTabs ? '1' : '0'; ?>">
              <div class="th3-qs-tabs" id="detailQsTabs" role="tablist" aria-label="示例语言">
                <?php foreach ($qsSamples as $qi => $qs): ?>
                <button type="button"
                        class="th3-pill<?php echo $qi === 0 ? ' is-active' : ''; ?>"
                        role="tab"
        ...
        <div class="th3-panel card" data-th3-panel="playground" role="tabpanel" id="detailPlayground"<?php echo $th3FirstTab === 'playground' ? '' : ' hidden'; ?>>
          <h2 class="font-display font-semibold text-lg m-0 mb-4">在线测试</h2>
          <?php if ($isDisabled): ?>
            <div class="th3-alert th3-alert--danger">接口已禁用，暂不可测试。</div>
          <?php elseif ($isMaintenance): ?>
            <div class="th3-alert th3-alert--warn">维护中，暂不可测试。</div>
          <?php elseif ($endpointDisplay === ''): ?>
            <p class="text-fg-2 m-0">未配置调用地址，无法测试。</p>
          <?php else: ?>
            <div class="th3-pg-grid">
              <div class="th3-pg-pane">
                <div class="th3-field-label">请求地址</div>
                <div class="th3-endpoint font-mono text-sm" id="pgUrlPreview"><?php echo vs_e($endpointDisplay); ?></div>

                <?php if (count($methods) > 1): ?>
                  <div class="th3-field-label mt-4">Method</div>
                  <div class="th3-method-row" id="pgMethodSelector">
                    <?php foreach ($methods as $i => $m): ?>
                      <button type="button" class="th3-pill<?php echo $i === 0 ? ' is-active' : ''; ?>" data-method="<?php echo vs_e(strtoupper(trim((string) $m))); ?>"><?php echo vs_e(strtoupper(trim((string) $m))); ?></button>
                    <?php endforeach; ?>
                  </div>
                <?php else: ?>
                ...
                <div class="th3-field-label mt-4">参数</div>
                <div id="pgParamsWrap" class="th3-pg-params">
                  <?php if ($hasParamsTable): ?>
                    <?php foreach ($paramsList as $p): ?>
        ...
        <div class="th3-panel card" data-th3-panel="feedback" role="tabpanel" id="detailFeedbackCard"
             ...
             <?php echo $th3FirstTab === 'feedback' ? '' : 'hidden'; ?>>
          ...
          <?php else: ?>
            <form id="detailFeedbackForm" class="th3-form" method="post" action="" novalidate>
      ...
      <?php if ($disclaimerBody !== ''): ?>
        <section class="th3-panel card mt-8" id="detailDisclaimer">
