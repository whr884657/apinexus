...
<section class="th3-page th3-article-detail">
  <div class="th3-page__inner max-w-3xl mx-auto px-4 sm:px-5 lg:px-8">
    ...
    <p class="th3-page__lead text-fg-2">
      <?php echo vs_e($article['createtime']); ?> · 阅读 <?php echo vs_e($article['views_label']); ?>
    </p>

    <?php if (!empty($article['cover'])): ?>
      <img class="th3-article-cover card mt-6" src="<?php echo vs_e($article['cover']); ?>"
           alt="<?php echo vs_e($article['title']); ?>"
           width="920" height="360" loading="lazy" decoding="async" referrerpolicy="no-referrer">
    <?php endif; ?>

    <article class="th3-panel card mt-8">
      <div class="markdown-body vs-md-body th3-md" data-vs-md="desktop">
        <?php echo $articleBodyHtml; ?>
      </div>
    </article>

    <section class="th3-panel card mt-8 th3-cmt" id="articleComments" data-content-id="<?php echo (int) $articleId; ?>">
      ...
      <?php if (!$commentsReady): ?>
        <div class="th3-notice mt-4">评论功能尚未就绪，请站长完成数据库结构更新。</div>
      <?php else: ?>
        <div class="th3-cmt-composer mt-6" id="articleCmtComposer">
          <div class="th3-cmt-quote" id="articleCmtQuote" hidden>
            ...
            <button type="button" class="btn-ghost th3-cmt-quote__clear" id="articleCmtQuoteClear" aria-label="取消引用">×</button>
          </div>
        ...
        <div class="th3-cmt-list mt-8" id="articleCmtList" aria-live="polite">
          <?php if ($commentCount === 0): ?>
            <div class="th3-notice" id="articleCmtEmpty">还没有评论，来抢沙发吧。</div>
          <?php else: ?>
              ...
              <div class="th3-cmt-item<?php echo !empty($c['ispinned']) ? ' is-pinned' : ''; ?>" data-cmt-id="<?php echo (int) $c['id']; ?>" id="cmt-<?php echo (int) $c['id']; ?>">
                <<?php echo $tag; ?> class="th3-cmt-avatar-wrap"<?php echo $attrs; ?>>
                  <?php if (!empty($c['avatar_url'])): ?>
                    <img class="th3-cmt-avatar" src="<?php echo vs_e($c['avatar_url']); ?>" alt="" width="40" height="40" loading="lazy" referrerpolicy="no-referrer">
                  <?php else: ?>
...
<section class="th3-page">
  <div class="th3-page__inner max-w-7xl mx-auto px-4 sm:px-5 lg:px-8">
    ...
    <?php if (count($articles) === 0): ?>
      <div class="th3-panel card mt-8 text-fg-2">暂无已发布文章。</div>
    <?php else: ?>
      <div class="th3-article-grid grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
        <?php foreach ($articles as $a): ?>
          <a class="th3-article-card card" href="<?php echo vs_e(vs_path_resource_url('articles', $a['id'])); ?>">
            <?php if (!empty($a['cover'])): ?>
              <img class="th3-article-card__cover" src="<?php echo vs_e($a['cover']); ?>" alt="" width="480" height="200" loading="lazy" decoding="async" referrerpolicy="no-referrer">
            <?php endif; ?>
