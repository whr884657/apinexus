...
th3_auth_shell_start('欢迎回来', '登录后继续使用平台能力');
?>
...
<form id="loginForm" method="post" action="" novalidate data-login-mode="password">
    ...
    <div id="modePassword" data-mode-panel="password">
        <div class="th3-auth__field">
            <input class="th3-auth__input" id="username" name="username" type="text" placeholder="用户名或邮箱" autocomplete="username" maxlength="64" required aria-label="用户名或邮箱">
        </div>
        <div class="th3-auth__field">
            <div class="th3-auth__pw-wrap">
                <input class="th3-auth__input" id="password" name="password" type="password" placeholder="密码" autocomplete="current-password" maxlength="64" required aria-label="密码">
    ...
    <div id="modeCode" data-mode-panel="code" hidden>
        <div class="th3-auth__field">
            <input class="th3-auth__input" id="email" name="email" type="email" placeholder="注册邮箱" autocomplete="email" maxlength="64" aria-label="注册邮箱" <?php echo $mailEnabled ? '' : 'disabled'; ?>>
        </div>
    </div>
    ...
    <?php if ($registerOpen): ?>
    <div class="th3-auth__foot">还没有账号？<a href="<?php echo vs_e($base); ?>/user/register">立即注册</a></div>
    <?php endif; ?>
</form>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (mode === 'code') {
            ...
            if (window.th3AuthSetLoading) window.th3AuthSetLoading(loginBtn, true);
            ...
            }).finally(function () { if (window.th3AuthSetLoading) window.th3AuthSetLoading(loginBtn, false); });
            return;
        }
        ...
        if (window.VsCaptcha && window.VsCaptcha.enabled && window.VsCaptcha.ensure) {
            window.VsCaptcha.ensure(form).then(runLogin).catch(function (err) {
                showMessage((err && err.message) || '请先完成行为验证', 'error');
                if (window.th3AuthSetLoading) window.th3AuthSetLoading(loginBtn, false);
            });
        } else { runLogin(); }
    });
})();
