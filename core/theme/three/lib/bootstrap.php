...
if (!function_exists('th3_resolve_media_url')) {
    /**
     * @param string $url
     * @return string
     */
    function th3_resolve_media_url($url)
    {
        $url = trim((string) $url);
        if ($url === '') {
            return '';
...
if (!function_exists('th3_social_icon_url')) {
    /**
     * @param string $type
     * @return string
     */
    function th3_social_icon_url($type)
    {
        $map = array(
            'wechat'  => 'wechat.svg',
            'qq'      => 'qq.svg',
        ...
        return ThemeManager::assetUrl('three', 'assets/img/' . $map[$type]);
    }
}

if (!function_exists('th3_footer_social_items')) {
    /**
     * 页脚社交图标（最多 3 个，由 theme.json 配置）
     *
     * @return array<int, array<string, string>>
     */
    function th3_footer_social_items()
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $item = array(
                'type'  => $type,
                'icon'  => $icon,
                'label' => th3_social_label($type),
            );

            if ($type === 'wechat' || ($type === 'qq' && $qqMode === 'qrcode')) {
                $qr = th3_resolve_media_url($value);
                if ($qr === '') {
                    continue;
                }
                $item['mode'] = 'qrcode';
...
if (!function_exists('th3_social_label')) {
    /**
     * @param string $type
     * @return string
     */
    function th3_social_label($type)
    {
        $labels = array(
            'wechat'  => '微信',
            'qq'      => 'QQ',
...
if (!function_exists('th3_render_footer_social')) {
     ...
     * @return void
     */
    function th3_render_footer_social()
    {
        $items = th3_footer_social_items();
        if (count($items) === 0) {
            return;
        }

        echo '<div class="th3-footer-social" role="list">';
        foreach ($items as $item) {
            ...
            if ($mode === 'qrcode') {
                ...
                echo '<div class="th3-social-item th3-social-item--qr" role="listitem">';
                echo '<button type="button" class="th3-social-btn" aria-label="' . vs_e($label) . '">';
                echo '<img src="' . vs_e($icon) . '" alt="" width="18" height="18" loading="lazy" decoding="async">';
                echo '</button>';
                echo '<span class="th3-social-qr-pop" role="tooltip">';
            ...
            echo '<a class="th3-social-item th3-social-btn" role="listitem" href="' . vs_e($href) . '"' . $target . ' aria-label="' . vs_e($label) . '">';
            echo '<img src="' . vs_e($icon) . '" alt="" width="18" height="18" loading="lazy" decoding="async">';
            echo '</a>';
        }
        echo '</div>';
...
if (!function_exists('th3_emit_console_brand_script')) {
    /**
     * 主题三专用：控制台品牌脚本走站内根路径（符合《前端页面渲染与源码规范》）
     *
     * @return void
     */
    function th3_emit_console_brand_script()
    {
