...
(function () {
    ...
    function renderItem(c) {
        ...
        var nameHtml = website
            ? '<a class="th5-cmt-name" href="' + esc(website) + '" target="_blank" rel="noopener noreferrer">' + esc(name) + '</a>'
            : '<span class="th5-cmt-name">' + esc(name) + '</span>';
        var avatarInner = avatar
            ? '<img class="th5-cmt-avatar" src="' + esc(avatar) + '" alt="" width="40" height="40" loading="lazy" referrerpolicy="no-referrer">'
            : '<span class="th5-cmt-avatar th5-cmt-avatar--letter">' + esc(name.charAt(0)) + '</span>';
        var avatarWrap = website
            ? '<a class="th5-cmt-avatar-wrap" href="' + esc(website) + '" target="_blank" rel="noopener noreferrer">' + avatarInner + '</a>'
            : '<div class="th5-cmt-avatar-wrap">' + avatarInner + '</div>';
        var refHtml = '';
        if (c.parent && c.parent.id) {
            refHtml = '<button type="button" class="th5-cmt-ref" data-jump="' + esc(c.parent.id) + '">'
                + '<span class="th5-cmt-ref__name">' + esc(c.parent.nickname || '') + '</span>'
                + '<span class="th5-cmt-ref__text">' + esc(c.parent.excerpt || '') + '</span></button>';
        }
        var replyHtml = c.reply
            ? '<div class="th5-cmt-admin-reply"><span class="th5-cmt-admin-reply__label">管理员回复</span>' + nl2br(c.reply) + '</div>'
            : '';
        var excerpt = (c.body || '').replace(/\s+/g, ' ').slice(0, 80);
        return '<div class="th5-cmt-item' + pin + '" data-cmt-id="' + esc(c.id) + '" id="cmt-' + esc(c.id) + '">'
            + avatarWrap
            + '<div class="th5-cmt-main">'
            + '<div class="th5-cmt-meta">' + nameHtml
            + (c.ispinned ? '<span class="tag hot">置顶</span>' : '')
            + '<span class="th5-cmt-time text-xs text-muted">' + esc(c.createtime_short || '') + '</span></div>'
            + refHtml
            + '<div class="th5-cmt-body">' + nl2br(c.body || '') + '</div>'
            + replyHtml
            + '<div class="th5-cmt-actions"><button type="button" class="btn-ghost text-sm article-cmt-reply-btn"'
    ...
    if (emojiBtn && emojiPanel) {
        ...
        document.addEventListener('click', function (e) {
            if (!emojiPanel.hidden && !e.target.closest('.th5-cmt-emoji-wrap')) {
                emojiPanel.hidden = true;
                emojiBtn.setAttribute('aria-expanded', 'false');
            }
        });
    ...
    document.addEventListener('click', function (e) {
        ...
        var jumpBtn = e.target.closest('.th5-cmt-ref[data-jump]');
        if (jumpBtn) {
