...
| Redis 缓存 | — | `RedisService` / `RedisCache` / `DashboardStats` | `admin/system/redis.php`、`admin/index.php`、`admin/screen.php` | 后台专用 | **业务缓存已接入**（公开接口 / 前台展示 / 分类 / 日志分页 / 限流 / 控制台与大屏分层 TTL） |
